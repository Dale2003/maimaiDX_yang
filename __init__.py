...
from .libraries.maimaidx_ap_50 import *
from .libraries.maimaidx_app_50 import *
from .libraries.maimaidx_worst_50 import *
from .libraries.maimaidx_random_50 import *
from .libraries.maimaidx_yang_50 import *
from .libraries.maimaidx_star_b50 import *
from .libraries.maimaidx_cunzhi_50 import *
from .libraries.maimaidx_suoxie_50 import *
from .libraries.maimaidx_aaa_50 import *
from .libraries.maimaidx_sss_50 import *
from .libraries.maimaidx_ss_50 import *
...
ap50 = on_command('ap50', aliases={'AP50'}, priority=5)
app50 = on_command('ap+50', aliases={'app50','AP+50','理论50','理论b50','理论B50'}, priority=5)
worst50 = on_command('w50', aliases={'W50','worst50','最差50','最低50','丢人50','丢人b50'}, priority=5)
random50 = on_command('r50', aliases={'R50','random50','随机50','随机b50','随机B50','sj50','sjb50'}, priority=5)
cunzhi50 = on_command('寸止50', aliases={'寸止b50','c50','cb50','czb50','C50'}, priority=5)
suoxie50 = on_command('锁血50', aliases={'锁血b50','s50','sxb50','sx50','S50'}, priority=5)
aaa50 = on_command('a50', aliases={'AAA50','aaa50','A50','越级50'}, priority=5)
sss50 = on_command('sss50', aliases={'SSS50','鸟50','N50','n50','没鸟加的50','没鸟加50'}, priority=5)
ss50 = on_command('ss50', aliases={'SS50','D50','d50','9950','没鸟的50','没鸟50'}, priority=5)
starb50 = on_regex(r'^([一二两三四五])星b50\s?(.+)?', priority=5)
...
level_achievement_list_dx = on_regex(r'^([0-9]+\+?)dx分数列表\s?([0-9]+)?\s?(.+)?', priority=5)
level_achievement_list_diff = on_regex(r'^([绿黄红紫白])([0-9]+\+?)分数列表\s?([0-9]+)?\s?(.+)?', priority=5)
yang_b50 = on_command('杨氏b50', aliases={'yangb50','yangB50','yb50'}, priority=5)
rating_ranking = on_command('查看排名', aliases={'查看排行'}, priority=5)
player_ranking = on_command('我有多菜', aliases={'查看玩家排名'}, priority=5)
...
async def _(event: MessageEvent, matcher: Matcher, arg: Message = CommandArg()):
    ...
    if _q := get_at_qq(arg):
        qqid = _q
    await matcher.finish(await generate_worst_50(qqid, username), reply_message=True)
...
async def _(event: MessageEvent, matcher: Matcher, arg: Message = CommandArg()):
    ...
    if _q := get_at_qq(arg):
        qqid = _q
    await matcher.finish(await generate_cunzhi_50(qqid, username), reply_message=True)
...
async def _(event: MessageEvent, matcher: Matcher, arg: Message = CommandArg()):
    ...
    if _q := get_at_qq(arg):
        qqid = _q
    await matcher.finish(await generate_suoxie_50(qqid, username), reply_message=True)
...
async def _(event: MessageEvent, matcher: Matcher, arg: Message = CommandArg()):
    ...
    if _q := get_at_qq(arg):
        qqid = _q
    await matcher.finish(await generate_aaa_50(qqid, username), reply_message=True)
...
async def _(event: MessageEvent, matcher: Matcher, arg: Message = CommandArg()):
    ...
    if _q := get_at_qq(arg):
        qqid = _q
    await matcher.finish(await generate_sss_50(qqid, username), reply_message=True)
...
async def _(event: MessageEvent, matcher: Matcher, arg: Message = CommandArg()):
    ...
    if _q := get_at_qq(arg):
        qqid = _q
    await matcher.finish(await generate_ss_50(qqid, username), reply_message=True)
...
async def _(bot: Bot, event: MessageEvent, match: Tuple = RegexGroup()):
    ...
    if get_at_qq(event.get_message()):
        username = None

    data = await generate_star_50(qqid, star, username)
    await starb50.finish(data, reply_message=True)
...
async def _(bot: Bot, event: MessageEvent, match: Tuple = RegexGroup()):
    ...
    data = await level_achievement_list_data(qqid, username, rating, page, nickname, bot, event)
    await level_achievement_list.finish(data, reply_message=True)

@level_achievement_list_dx.handle()
async def _(bot: Bot, event: MessageEvent, match: Tuple = RegexGroup()):
    qqid = get_at_qq(event.get_message()) or event.user_id
    ...
    data = await level_achievement_list_data_dx(qqid, username, rating, page, nickname, bot, event)
    await level_achievement_list_dx.finish(data, reply_message=True)

@level_achievement_list_diff.handle()
async def _(bot: Bot, event: MessageEvent, match: Tuple = RegexGroup()):
    qqid = get_at_qq(event.get_message()) or event.user_id
    ...
    if rating not in levelList:
        await level_achievement_list_diff.finish('无此等级', reply_message=True)
    elif match[3]:
        nickname = match[3]
    ...
    data = await level_achievement_list_diff_data(qqid, username, rating, diff, page, nickname, bot, event)
    await level_achievement_list_diff.finish(data, reply_message=True)
...
async def _(event: MessageEvent, matcher: Matcher, arg: Message = CommandArg()):
    ...
    if _q := get_at_qq(arg):
        qqid = _q
    await matcher.finish(await generate_yang50(qqid, username), reply_message=True)
    '''
    qqid = get_at_qq(event.get_message()) or event.user_id
    nickname = ''
    username = None
    ...
    data = await yang_b50_data(qqid, username, nickname, bot, event)
    await yang_b50.finish(data, reply_message=True)
    '''
...
async def _(bot: Bot, event: MessageEvent):
    ...
    data = await player_ranking_data(qqid, username, nickname)
    await player_ranking.finish(data, reply_message=True)
