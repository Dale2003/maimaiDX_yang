...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        # 选出achievements大于100.4小于100.5以及大于99.9小于100的歌曲
        self.sdBest = [i for i in self.sdBest if 99.9 <= i.achievements < 100.0 or 100.4 <= i.achievements < 100.5]
        self.dxBest = [i for i in self.dxBest if 99.9 <= i.achievements < 100.0 or 100.4 <= i.achievements < 100.5]

        # 计算与100.5或100的差值
        for i in self.sdBest:
            if i.achievements < 100:
                i.dis = 100 - i.achievements
            else:
                i.dis = 100.5 - i.achievements
        for i in self.dxBest:
            if i.achievements < 100:
                i.dis = 100 - i.achievements
            else:
                i.dis = 100.5 - i.achievements
        ...
        #按ra从高到低排序，取前35首
        self.sdBest = sorted(self.sdBest, key=lambda x: x.dis, reverse=False)[:35]
        self.dxBest = sorted(self.dxBest, key=lambda x: x.dis, reverse=False)[:15]
    ...
    async def draw(self):
        ...
        self._tb.draw(847, 295, 26, f'(cunzhi) B35: {sdrating} + B15: {dxrating} = {self.Rating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_cunzhi_50(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
