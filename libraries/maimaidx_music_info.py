...
from ..config import *
from .image import DrawText, draw_gradient, image_to_base64
    ...
    try:
        ...
        for _data in player_data:
            ...
            for _ in range(dxnum):
                im.alpha_composite(dxstar[dxtype], (DXSTAR_DEST[dxnum] + 20 * _, 550 + y * lv - 12))
            ...
            if _data['fc'] == 'fcp':
                y8 += 1
            elif _data['fc'] == 'ap' or _data['fc'] == 'app':
                y8 += 2
            if float(_data['achievements']) >= 100.98:
                y8 += 3
            elif float(_data['achievements']) >= 100.95:
                y8 += 2
            elif float(_data['achievements']) >= 100.9:
                y8 += 1
            y8 += max(dxnum - 2, 0)

            yrating = float(_data['ds']) * min((max(100*(float(_data['achievements'])-100), 0)), (100 * float(_data['dxScore']) / _dxscore))/100


            p, s = f'{_data["achievements"]:.4f}'.split('.')
            r = tb.get_box(p, 34)
            tb.draw(90, 545 + y * lv, 30, ds, anchor='mm')
            tb.draw(175 + 3, 567 + y * lv - 10, 34, p, TEXT_COLOR[lv], 'ld')
            tb.draw(175 + 3 + r[2], 565 + y * lv - 10, 28, f'.{s}%', TEXT_COLOR[lv], 'ld')
            tb.draw(177, 563 + y * lv, 20, f'DX rate: {dxscore/_dxscore*100:.3f}%', TEXT_COLOR[lv], 'lm')
            tb.draw(550, (540 if dxnum != 0 else 548) + y * lv - 10, 18, f'{dxscore}/{_dxscore}', TEXT_COLOR[lv], 'mm')
            tb.draw(790, 545 + y * lv, 32, ra, TEXT_COLOR[lv], 'mm')
            im.alpha_composite(Image.open(maimaidir / 'yang.png').resize((18,18)), (557, y * lv + 558))
            tb.draw(574, 565 + y * lv + 2, 16, f'x{y8}', TEXT_COLOR[lv], 'lm')
            tb.draw(530, 565 + y * lv + 2, 16, f'{yrating:.3f}', TEXT_COLOR[lv], 'mm')

        sy.draw(450, 1105, 20, f'杨氏rating计算方式为：谱面定数*min(100*(达成率-100),dx分/总dx分)，最低为0', (159, 81, 220, 255), 'mm', 2, (255, 255, 255, 255))
        sy.draw(450, 1130, 20, f'8分法计算方式为：FC+，AP，100.9，100.95，100.98，三星，四星，五星各计一分。', (159, 81, 220, 255), 'mm', 2, (255, 255, 255, 255))
        sy.draw(450, 1155, 20, f'仅供娱乐，请玩家合理参考，解释权归杨树森6所有。', (159, 81, 220, 255), 'mm', 2, (255, 255, 255, 255))
        sy.draw(450, 1180, 20, f'改编自：Yuri-YuzuChaN，作者：宇航员Dale，Bot：宇航员猫娘', (159, 81, 220, 255), 'mm', 2, (255, 255, 255, 255))
        # sy.draw(450, 1180, 20, f'Designed by Yuri-YuzuChaN | Generated by {maiconfig.botName} BOT', (159, 81, 220, 255), 'mm', 2, (255, 255, 255, 255))
        msg = MessageSegment.image(image_to_base64(im))
