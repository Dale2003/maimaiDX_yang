...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None, level: Optional[str] = None, page: Optional[int] = None, diff_index: Optional[int] = None):
        ...
        self.sdBest = [i for i in self.records if i.level == self.level and i.level_index == self.diff_index]
        self.dxBest = []

        #按ra从高到低排序，取前35首
        self.sdBest = sorted(self.sdBest, key=lambda x: x.achievements, reverse=True)[page * SONGS_PER_PAGE - SONGS_PER_PAGE:page * SONGS_PER_PAGE]
    ...
    async def draw(self):
        ...
        if self.diff_index == 0:
            diff = 'Basic'
        elif self.diff_index == 1:
            diff = 'Advanced'
        elif self.diff_index == 2:
            diff = 'Expert'
        elif self.diff_index == 3:
            diff = 'Master'
        else:
            diff = 'Re:Master'
        self._tb.draw(847, 295, 28, f'{diff} LV{self.level} score list, No.{self.page} page.', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_list_diff(qqid: Optional[int] = None, username: Optional[str] = None, level: Optional[str] = None, page: Optional[int] = None, diff_index: Optional[int] = None) -> MessageSegment:
    try:
        ...
        draw_best = DrawBest(mai_info, qqid, level, page, diff_index)
