...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        self.sdBest = [i for i in self.sdBest if i.achievements < 100.5]
        self.dxBest = [i for i in self.dxBest if i.achievements < 100.5]
    ...
    async def draw(self):
        ...
        self._tb.draw(847, 295, 24, f'(Below SSS+) B35: {sdrating} + B15: {dxrating} = {self.Rating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_sss_50(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
