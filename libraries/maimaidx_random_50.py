...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        #按ra从高到低排序，取前35首
        self.sdBest = sorted(self.sdBest, key=lambda x: x.ra, reverse=True)
        self.dxBest = sorted(self.dxBest, key=lambda x: x.ra, reverse=True)
    ...
    async def draw(self):
        ...
        self._tb.draw(847, 295, 27, f'(Random) B35: {sdrating} + B15: {dxrating} = {self.Rating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
