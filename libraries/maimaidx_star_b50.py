...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None, star: Optional[int] = None):
        ...
        self.sdBest_origin = [i for i in self.records if mai.total_list.by_id(str(i.song_id)).basic_info.is_new == False]
        self.dxBest_origin = [i for i in self.records if mai.total_list.by_id(str(i.song_id)).basic_info.is_new == True]

        self.sdBest_origin = sorted(self.sdBest_origin, key=lambda x: x.ra, reverse=True)
        self.dxBest_origin = sorted(self.dxBest_origin, key=lambda x: x.ra, reverse=True)

        sd_num = 0
        dx_num = 0

        # 选出starnum大于等于star的records
        for i in self.sdBest_origin:
            if sd_num >= 35:
                break
            dxscore = sum(mai.total_list.by_id(str(i.song_id)).charts[i.level_index].notes) * 3
            dxrate = i.dxScore / dxscore * 100
            dxtype, dxnum = dxScore(dxrate)
            if dxnum >= int(star):
                sd_num += 1
                self.sdBest.append(i)
        for i in self.dxBest_origin:
            if dx_num >= 15:
                break
            dxscore = sum(mai.total_list.by_id(str(i.song_id)).charts[i.level_index].notes) * 3
            dxrate = i.dxScore / dxscore * 100
    ...
    async def draw(self):
        ...
        self._tb.draw(847, 295, 25, f'({self.star}-star) B35: {sdrating} + B15: {dxrating} = {self.Rating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_star_50(qqid: Optional[int] = None, star: Optional[int] = None, username: Optional[str] = None) -> Union[str, MessageSegment]:
    try:
        if username:
            qqid = None
        ...
        draw_best = DrawBest(mai_info, qqid, star)
