...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        # 把ra=0的歌曲去掉
        self.sdBest = [i for i in self.sdBest if i.ra != 0]
        self.dxBest = [i for i in self.dxBest if i.ra != 0]

        #按ra从高到低排序，取前35首
        self.sdBest = sorted(self.sdBest, key=lambda x: x.ra, reverse=False)[:35]
        self.dxBest = sorted(self.dxBest, key=lambda x: x.ra, reverse=False)[:15]
    ...
    async def draw(self):
        ...
        self._tb.draw(847, 295, 27, f'(Worst) B35: {sdrating} + B15: {dxrating} = {self.Rating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_worst_50(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
