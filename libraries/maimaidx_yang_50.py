...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        # 选出achievement大于100的records
        self.sdBest = [i for i in self.sdBest if i.achievements >= 100]
        self.dxBest = [i for i in self.dxBest if i.achievements >= 100]

        # 计算yangrating
        for i in self.sdBest + self.dxBest:
            total_dxscore = sum(mai.total_list.by_id(str(i.song_id)).charts[i.level_index].notes) * 3
            dxrate = i.dxScore/ total_dxscore * 100
            '''
            if dxrate <= 85:
                starnum = 0
            elif dxrate <= 90:
                ...
                starnum = 5
            '''
            yrating = i.ds* min((max(100*(i.achievements-100), 0)), dxrate)/100
            i.yrating = yrating

        #按yrating从高到低排序，取前35首
        self.sdBest = sorted(self.sdBest, key=lambda x: x.yrating, reverse=True)[:35]
        self.dxBest = sorted(self.dxBest, key=lambda x: x.yrating, reverse=True)[:15]

        # 把sdBest和dxBest的所有歌曲ra加起来
        self.Rating = int((sum([_.yrating for _ in self.sdBest]) + sum([_.yrating for _ in self.dxBest]))/50*1000)
    ...
    def _findRaPic(self) -> str:
        if self.Rating < 100:
            ...
            num = '06'
        elif self.Rating < 9000:
            num = '07'
        elif self.Rating < 10000:
            num = '08'
        elif self.Rating < 11000:
            num = '09'
    ...
    async def whiledraw(self, data: List[ChartInfo], type: bool) -> Image.Image:
        ...
        for num, info in enumerate(data): 
            ...
            self._im.alpha_composite(Image.open(maimaidir / 'yang.png').resize((20,20)), (x + 313, y + 78))
            ...
            self._tb.draw(x + 157, y + 80, 21, f'{info.ds} x {(info.yrating/info.ds*100):.2f}%', TEXT_COLOR[info.level_index], anchor='lm')
            self._tb.draw(x + 340, y + 20, 23, f'{info.yrating:.3f}', TEXT_COLOR[info.level_index], anchor='mm')
            self._tb.draw(x + 350, y + 85, 21, f'x {y8}', TEXT_COLOR[info.level_index], anchor='mm')
    ...
    async def draw(self):
        ...
        for n, i in enumerate(Rating):
            if n < 2:
                self._im.alpha_composite(Image.open(maimaidir / f'UI_NUM_Drating_{i}.png').resize((26, 32)), (759 + 22 * n, 137))
            else:
                self._im.alpha_composite(Image.open(maimaidir / f'UI_NUM_Drating_{i}.png').resize((26, 32)), (764 + 22 * n, 137))
        ...
        self._im.alpha_composite(Image.open(maimaidir / 'dot.png').resize((13,13)), (801, 153))
        ...
        # self._siyuan.draw(803, 160, 30, f'.', (255, 224, 67, 255), 'lm')
        sdrating, dxrating = sum([_.yrating for _ in self.sdBest])/35, sum([_.yrating for _ in self.dxBest])/15
        self._tb.draw(847, 295, 28, f'B35: {sdrating:.3f} + B15: {dxrating:.3f} = {self.Rating/1000}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_yang50(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
