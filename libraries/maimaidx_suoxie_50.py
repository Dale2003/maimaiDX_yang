...
class DrawBest:
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        # 选出achievements大于100.0小于100.1以及大于100.5小于100.6的歌曲
        self.sdBest = [i for i in self.sdBest if 100 <= i.achievements <= 100.1 or 100.5 <= i.achievements <= 100.6]
        self.dxBest = [i for i in self.dxBest if 100 <= i.achievements <= 100.1 or 100.5 <= i.achievements <= 100.6]

        # 计算与100.5或100的差值
        for i in self.sdBest:
            if i.achievements < 100.5:
                i.dis = abs(100 - i.achievements)
            else:
                i.dis = abs(100.5 - i.achievements)
    ...
    async def draw(self):
        ...
        self._tb.draw(847, 295, 26, f'(suoxie) B35: {sdrating} + B15: {dxrating} = {self.Rating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_suoxie_50(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
